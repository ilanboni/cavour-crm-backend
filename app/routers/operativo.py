...
async def lista_comunicazioni(
    ...
    i = 1
    if cliente_id:
        conditions.append(f"c.cliente_id = ${i}")
        params.append(cliente_id)
        i += 1
    if immobile_id:
        conditions.append(f"c.immobile_id = ${i}")
    ...
    async with db.acquire() as conn:
        rows = await conn.fetch(f"""
            ...
            WHERE {where} ORDER BY c.data_ora DESC LIMIT ${i}
        """, *params, limit)
    ...
    async with db.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO public.comunicazioni
                (cliente_id, immobile_id, immobile_esterno_id, tipo,
                 testo, canale, esito, direzione, creato_da)
            VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'ilan') RETURNING *
        """, comm.cliente_id, comm.immobile_id, comm.immobile_esterno_id,
            comm.tipo, comm.testo, comm.canale, comm.esito, comm.direzione)
    return dict(row)

@comm_router.delete("/{comm_id}")
async def elimina_comunicazione(comm_id: int, db: asyncpg.Pool = Depends(get_db)):
    async with db.acquire() as conn:
        await conn.execute("DELETE FROM public.comunicazioni WHERE id = $1", comm_id)
    return {"success": True}
...
async def lista_appuntamenti(
    ...
    if completato is not None:
        conditions.append(f"a.completato = ${i}")
        params.append(completato)
        i += 1
    where = " AND ".join(conditions)
    async with db.acquire() as conn:
        rows = await conn.fetch(f"""
            ...
            WHERE {where} ORDER BY a.data_ora ASC LIMIT ${i}
        """, *params, limit)
    return [dict(r) for r in rows]
    ...
    async with db.acquire() as conn:
        row = await conn.fetchrow("""
            INSERT INTO public.appuntamenti
                (cliente_id, immobile_id, immobile_esterno_id, data_ora, luogo, tipo, note)
            VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *
        """, appt.cliente_id, appt.immobile_id, appt.immobile_esterno_id,
            appt.data_ora, appt.luogo, appt.tipo, appt.note)
    return dict(row)

@appt_router.patch("/{appt_id}")
async def aggiorna_appuntamento(
    appt_id: int, appt: AppuntamentoUpdate, db: asyncpg.Pool = Depends(get_db)
):
    ...
    async with db.acquire() as conn:
        row = await conn.fetchrow(
            f"UPDATE public.appuntamenti SET {set_clause} WHERE id = $1 RETURNING *",
            appt_id, *list(updates.values()))
    if not row:
...
async def lista_documenti(
    ...
    i = 1
    if immobile_id:
        conditions.append(f"d.immobile_id = ${i}")
        params.append(immobile_id)
        i += 1
    if cliente_id:
        conditions.append(f"d.cliente_id = ${i}")
        params.append(cliente_id)
        i += 1
    where = " AND ".join(conditions)
    async with db.acquire() as conn:
...
async def elimina_documento(doc_id: int, db: asyncpg.Pool = Depends(get_db)):
    async with db.acquire() as conn:
        await conn.execute("DELETE FROM public.documenti WHERE id = $1", doc_id)
    return {"success": True}
